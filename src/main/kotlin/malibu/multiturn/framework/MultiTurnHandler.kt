...
class MultiTurnHandler(
    ...
    fun handle(
        ...
        val expressionRoot = ExpressionRoot(
            req = multiTurnReq,
            ...
            .switchIfEmpty {
                topicState.getHeadIntends()
                    ...
                    .filterWhen { intend ->
                        ...
                        }.thenReturn(behaviorRegistry.expressionParser.evaluate(
                            expression = intend.triggerExpression,
                            expressionRoot = expressionRoot,
                            desiredResultType = Boolean::class
                        ) ?: false)
                    }
                    .singleOrEmpty()
            }
            .switchIfEmpty { Mono.error(RuntimeException("intend not trigger. intent: ${multiTurnReq.intent}")) }
            .flatMap { selectedIntend -> loadArgument(selectedIntend.getIntendArguments(), intendData).thenReturn(selectedIntend) } // argument 로딩
            .flatMap { selectedIntend ->
                if (selectedIntend.getTasks().isEmpty()) {
                    return@flatMap Mono.error(RuntimeException("task 가 등록되지 않았습니다. selectedIntend: $selectedIntend"))
                }

                if (selectedIntend.getTasks().size == 1) {
                    return@flatMap selectedIntend.getTasks().first().toMono()
                }

                selectedIntend.getTasks()
                    .toFlux()
                    .filterWhen { task ->
                        Mono.defer {
                            ...
                            expression = task.triggerExpression,
                            expressionRoot = expressionRoot,
                            desiredResultType = Boolean::class
                        ) ?: false)
                    }
            ...
            .flatMap { selectedTask ->
                val executableActions = selectedTask.getActions().filter { action ->
                    val enabled = action.enabledPredicate?.let { enabledPredicate ->
                        behaviorRegistry.expressionParser.evaluate(
                            expression = enabledPredicate,
                            expressionRoot = expressionRoot,
                            desiredResultType = Boolean::class
                        )
                    } ?: true
    ...
    private fun loadArgument(arguments: List<NameValue<Argument>>, intendData: IntendData): Mono<Void> {
        ...
        return arguments.toFlux()
            ...
            .doOnComplete { intendData.finishArgumentLoad = true }
            .then()
//            .collectMap()
    }
}